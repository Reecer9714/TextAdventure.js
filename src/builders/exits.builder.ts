...
export class ExitsBuilder {
  ...
  public add(exitName: string): ExitBuilder {
    this._exitBuilders[exitName] =
      this._exitBuilders[exitName] || new ExitBuilder(this._gameContext);

    return this._exitBuilders[exitName];
  }

  public build(): IExitCollection {
    const exitBuilders = this._savedExits
      ? this.createExitBuildersFromSavedExits()
      : this._exitBuilders;
